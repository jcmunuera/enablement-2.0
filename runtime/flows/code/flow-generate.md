...
### Mode A: Standalone (no DESIGN pipeline)

Discovery runs from scratch on the user prompt, resolving capabilities and modules.
...
### Mode B: DESIGN-seeded (from Implementation Design pipeline)

When CODE receives output from the DESIGN pipeline, the input is richer:

From DESIGN pipeline (per bounded context):
```yaml
# prompt.md          — Enriched prompt with BDD scenarios, aggregate definitions,
#                      integration context, capability table
# domain-api-spec.yaml — OpenAPI contract to implement (the Domain API)
# manifest.yaml      — Pre-resolved capabilities (NOT modules)
# aggregate-definitions.yaml — Full DDD model (entities, VOs, invariants)
# {aggregate}.feature — BDD scenarios (Gherkin, for test generation)
```

From User (external to DESIGN):
```yaml
...
### Discovery in Mode B: Merge with Precedence

When a DESIGN manifest is present, discovery follows this process:

```python
def discover_with_manifest(prompt: str, manifest: Manifest) -> DiscoveryResult:
    # 1. Load DESIGN-resolved capabilities as seed
    seed_capabilities = manifest.capabilities  # e.g., [persistence.systemapi, ...]
    
    # 2. Run normal discovery on prompt (may find additional capabilities)
    prompt_capabilities = discover_from_prompt(prompt)
    
    # 3. Merge: seed + prompt-discovered, no duplicates
    #    Seed has precedence (already validated by DESIGN pipeline)
    all_capabilities = merge_unique(seed_capabilities, prompt_capabilities)
    
    # 4. Resolve modules from final capability set
    #    This is ALWAYS done by CODE — DESIGN does NOT resolve modules
    modules = resolve_modules(all_capabilities, manifest.tech_stack)
    
    # 5. Resolve config flags
    config = resolve_config(all_capabilities, modules)
    
    return DiscoveryResult(
        features=all_capabilities,
        modules=modules,
        config=config,
        source="design-seeded"  # for traceability
    )
```

**Key rules:**
- DESIGN resolves **capabilities** (what the context needs)
- CODE resolves **modules** (how to implement each capability in the tech stack)
- Prompt discovery still runs — it may detect capabilities DESIGN didn't infer
  (e.g., user explicitly requests rate-limiter, which is a runtime concern)
- serviceName derived from context ID (e.g., card-management → card-management-api)
- basePackage derived from org convention + context ID
...
- [Flow: Implementation Design](../design/flow-implementation-design.md) - DESIGN pipeline (produces Mode B input)
